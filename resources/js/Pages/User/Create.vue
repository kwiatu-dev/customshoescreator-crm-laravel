<template>
  <form class="container mx-auto p-4" @submit.prevent="create">
    ...
    <section class="mt-8 flex flex-col justify-center md:grid md:grid-cols-6 gap-4">
      ...
      <div class="col-span-3">
        <label for="country" class="label">Koszty stałe</label>
        <input id="country" v-model="form.costs" type="number" class="input" />
        <FormError :error="form.errors.costs" />
      </div>

      <div class="col-span-3">
        <label for="country" class="label">Prowizja</label>
        <input id="country" v-model="form.commission" type="number" class="input" />
