<template>
  <button class="px-4 py-2 bg-gray-800 rounded-lg" @click="open">{{ toggle ? 'Filtry ↓' : 'Schowaj ↑' }}</button>
  <div id="dropdown" class="w-80 bg-gray-800 p-4 rounded-lg aboslute mt-2" :class="{'hidden': toggle}">
    <div class="flex flex-row flex-nowrap justify-between">
      <h6 class="text-gray-200 font-medium">Filtry</h6>
      <div class="flex flex-row flex-nowrap gap-4">
        <span class="text-indigo-500" @click="clear">
          Resetuj
        </span>
      </div>
    </div>
    ...
    <div id="accordion-flush" class="mt-4">
      ...
      <Price v-if="sections['price']" :form="form" :reload="reload" @filters-update="update" />
